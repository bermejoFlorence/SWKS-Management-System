...
$orgName = '';
if ($userId) {
    $stmt = $conn->prepare("SELECT o.org_name 
        FROM user u
...
// ====== GET PROFILE PIC DIRECT FROM DB ======
    ...
    <div class="ms-auto d-flex align-items-center" style="gap:16px;">
            ...
            <a href="#" class="position-relative" id="notifDropdownBtn" data-bs-toggle="dropdown" aria-expanded="false" style="display:inline-block; text-decoration:none; color:inherit;">
                ...
                <?php if ($notifCount > 0): ?>
                    <span id="notifBadge" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"
                          style="font-size:0.77em; min-width: 22px; z-index: 2; border: 2px solid #fff;">
        ...
        <div class="dropdown">
            <?php if ($profilePic): ?>
                ...
                <img src="<?= strpos($profilePic, 'http') === 0 ? $profilePic : '/swks/' . htmlspecialchars($profilePic) ?>?v=<?= time() ?>"  alt="Profile" class="rounded-circle" width="32" height="32" id="profileDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="object-fit:cover;cursor:pointer;">
            <?php else: ?>
                <!-- Fallback to icon -->
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow profile-dropdown" aria-labelledby="profileDropdown" style="min-width: 260px;">
                <li class="px-3 py-2 mb-1 user-info-area">
                    ...
                    <div style="font-size:0.97rem; color:#186a1a; font-weight: 500;">
                      <?= htmlspecialchars($orgName) ?> Adviser
                  </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body text-center pt-0">
        <div class="profile-icon d-inline-block rounded-circle p-1 shadow" style="margin-bottom:10px;">
          <?php if ($profilePic): ?>
            <img src="/swks/<?= htmlspecialchars($profilePic) ?>" alt="Profile" class="rounded-circle" width="76" height="76" style="object-fit:cover;">
          <?php else: ?>
        ...
        <div class="modal-body pb-2">
          ...
          <div class="text-center mb-3">
            <label for="profilePicInput" class="d-block mb-2" style="cursor:pointer;">
              <?php if ($profilePic): ?>
                <img src="<?= htmlspecialchars($profilePic) ?>" id="profilePicPreview" class="rounded-circle border" width="74" height="74" style="object-fit:cover;">
              <?php else: ?>
