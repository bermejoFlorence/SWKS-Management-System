...
// helper: detect inventory/borrow-type notifications
function is_inventory_notif($type){
  if (!$type) return false;
  // any type starting with these prefixes will go to inventory.php
  return (strpos($type, 'borrow_') === 0) || (strpos($type, 'inventory_') === 0);
}

if ($userId) {
  // keep your existing SELECT (forum-centric), we’ll just change link targets per type below
...
<li class="px-3 py-2 border-bottom bg-light fw-bold" style="font-size:1rem;">Notifications</li>
...
<?php if (count($notifications)): ?>
  <?php foreach ($notifications as $notif): ?>
    <?php
      ...
      // default: forum link (avoid dead link if no post_id)
      $href = !empty($notif['post_id'])
        ? "view_forum_post.php?post_id=".(int)$notif['post_id']."&notif_id=".(int)$notif['notification_id']
        : "inventory.php?notif_id=".(int)$notif['notification_id'];

      // build message + override link for each type
      if ($notif['type'] === 'forum_comment') {
        ...
        $notifMsg  = "<b>".htmlspecialchars($actorName)."</b> posted: "
                   . "<span class='text-success'>".htmlspecialchars($notif['title'])."</span>";
      ...
      /* 🔔 INVENTORY / BORROW: always open inventory.php */
      } elseif (is_inventory_notif($notif['type'])) {
        // keep whatever message you inserted (like your screenshot string)
        $notifMsg = htmlspecialchars($notif['message'] ?: 'Inventory update');
        $href     = "inventory.php?notif_id=".(int)$notif['notification_id'];

      } else {
        // fallback
        $notifMsg = htmlspecialchars($notif['message'] ?: 'Notification');
        // if no post, route to inventory to avoid 404
